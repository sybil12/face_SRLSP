...
% version1.1 回归器学习在SR之前，使用row patch进行计算 对应的faceSR.m
...
nTesting    = 5;                   % number of test sample
...
if exist(str,'file')
    ...
    tic
    PPs = trainP_faceSR(YH, YL, imrow, imcol, upscale,patch_size,overlap, lambda);
    toc
    save(str,'PPs')
    disp('Done.');
end
...
%     [im_SR] = faceSR(im_l,YH,YL,upscale,patch_size,overlap);
    [im_SR] = faceSR(im_l,PPs,imrow, imcol, upscale,patch_size,overlap);
    toc
