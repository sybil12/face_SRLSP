...
str = ['./data/YH_YL_CMDP' '_s' num2str(upscale) '.mat'];
...
str = ['./data/YH_YL_CMDP_dict-PP' '_s' num2str(upscale) '.mat'];
...
%% testing
testpath = './CMDP_test/';
imgDir  = dir([testpath '*.tif']); % 遍历所有tif格式文件
...
%     im_h = imread(strh);
    im_h = imread([testpath imgDir(TestImgIndex).name]); %读取每张图片
    im_h = im2double(im_h);
...
%     im_h = uint8(im_h);im_n = uint8(im_n);im_b = uint8(im_b);im_SR = uint8(im_SR);
    ...
    % save the result
    strw = strcat('./results/SRLSP-',num2str(TestImgIndex),'_dictbased_patchsz',num2str(patch_size),'_X',num2str(upscale),'.bmp');
